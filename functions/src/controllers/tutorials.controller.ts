      ...
      blobWriter.on("finish", async () => {
        ...
        if (fileObj.imageUrl.length !== 0 && fileObj.videoUrl.length !== 0) {
          db.collection(tutorialsCollection)
            .where("categoryNumber", "==", formData["categoryNumber"])
            .get()
            .then(async (result) => {
              // console.log(result)
              if (!result.empty) {
                ...
                let newData;
                let tutorialId = suid();
                let newImageName = fileObj.imageUrl
                  .split("o/")[1]
                  .split("?")[0];
                let newVideoName = fileObj.videoUrl
                  .split("o/")[1]
                  .split("?")[0];
                let data = {
                  id: tutorialId,
...
export let updateTutorial = async (
  ...
  try {
    ...
    let filesObj = {
      image: { content: Buffer, filename: "", contentType: String },
        ...
        db.collection(tutorialsCollection)
          ...
          .then(async (result) => {
            ...
            let index = oldData.findIndex(
              (tutorial) => tutorial.id === formData["tutorialId"]
            );
            let newData = [...oldData];
            newData[index] = {
              id: oldData[index].id,
